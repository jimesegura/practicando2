...
Procedure cargarArbol(var a:arbol);
	...
	begin
		...
		begin
			new(a);
			a^.dato.codigoP:=e.codPostal;
			a^.dato.lista:=nil;
			agregarLista(a^.dato.lista,e2);
			a^.hi:=nil;
			a^.hd:=nil;
		end
		else
			if(e.codPostal=a^.dato.codigoP)then
				agregarLista(a^.dato.lista,e2)
			else
				if(e.codPostal<a^.dato.codigoP)then
...
begin
	if(a<>nil)then begin
		imprimirArbol(a^.hi);
		writeln('CODIGO POSTAL: ',a^.dato.codigoP);
...
begin
	if(a<>nil)then begin
		if(cp=a^.dato.codigoP)then
			agregar(a^.dato.lista,lb)
		else 
			if(cp<a^.dato.codigoP)then
