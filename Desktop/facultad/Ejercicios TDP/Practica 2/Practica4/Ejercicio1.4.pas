...
{Contenga un módulo que reciba la estructura generada en el punto a y retorne el código de producto con mayor cantidad de unidades vendidas.}
  ...
  begin
    if(a<>nil)then 
    begin
		if (a^.dato.cantTotalUnidades>maxCant) then
			begin
...
estructura. }
  ...
  begin
    if(a=nil)then
		ObtenerCantidadCodigosMenores:=0
	else
	begin
		if(a^.dato.codigo<unCodigo)then
			ObtenerCantidadCodigosMenores:=ObtenerCantidadCodigosMenores+1;
		ObtenerCantidadCodigosMenores:=ObtenerCantidadCodigosMenores(a^.HI,unCodigo);
		ObtenerCantidadCodigosMenores:=ObtenerCantidadCodigosMenores(a^.HD,unCodigo);
	end;
  end;
